...
/**
 * Generate PDF to buffer (not file)
 * @param {function} generateContent - Function that receives doc and generates content
 * @returns {Promise<Buffer>} PDF buffer
 */
function generatePDFBuffer(generateContent) {
...
/**
 * Generate approval PDF document for a CR and upload to Azure Blob Storage
 * @param {string} crId 
...
async function generateApprovalPDF(crId) {
  ...
  const fileName = `${crId}-approval.pdf`;
  ...
  await prisma.document.create({
    data: {
      crId,
      fileName,
      filePath: uploadResult.url, // Store blob URL instead of local path
...
/**
 * Generate dynamic form PDF and upload to Azure Blob Storage
 * @param {string} crId 
 * @returns {Promise<string>} URL to generated PDF
 */
...
/**
 * Get PDF document info for download
 * @param {string} crId 
 * @param {string} type - 'approval' or 'form'
 * @returns {Promise<object>} Document info with URL
 */
async function getPDFInfo(crId, type = 'approval') {
  ...
  return {
    ...
    filePath: document.filePath, // This is now a blob URL
